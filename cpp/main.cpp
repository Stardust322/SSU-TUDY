...
svr.Get("/grade-input", [](const httplib::Request& req, httplib::Response& res) {
    std::string html = R"(<!DOCTYPE html>
<html lang="ko">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>SSU:TUDY - 성적 입력</title>
    <link rel="website icon" type="png" href="http://localhost:8080/ssutudy_logo.png">
    <style>
        body {
            font-family: 'Malgun Gothic', Arial, sans-serif;
...
<body>
    ...
    <div class="nav">
        ...
        <div class="nav-item-main">성적 입력</div>
        <div class="nav-item">게시판</div>
        <div class="nav-item">설정</div>
    </div>
    ...
    <div class="container">
        ...
        <div class="tab-container">
            <button class="tab active" onclick="switchTab('completed') ">완료된 과목 성적 입력</button>
            <button class="tab" onclick="switchTab('current') ">현재 수강 과목 추가</button>
        </div>
        
        <!-- 완료된 과목 성적 입력 폼 -->
        <div id="completed-section" class="form-section active">
            <div class="info-box">
            ...
            <form id="current-form">
                ...
                <div class="form-group">
                    <label for="current-category">
                        카테고리 
                        <span class="optional-label">(과목 주요 분야)</span>
                    </label>
    ...
    <script>
        ...
        document.getElementById('current-form').addEventListener('submit', function(e) {
            ...
            const formData = {
                ...
                grade: null,  // 현재 수강중이므로 성적 없음
                category: categoryValue || null,  // 카테고리가 선택되지 않으면 null
                status: 'current'
            };
    ...
    .then(response => {
        ...
        if (response.ok) {
            ...
            document.getElementById('error-message').textContent = '저장 중 오류가 발생했습니다.';
            document.getElementById('error-message').style.display = 'block';
            document.getElementById('success-message').style.display = 'none';
        }
    })
